...
```text
첫 번째 SQL은 __첫 번째 SQL은 JOIN된 행의 수__을 세고 있었다.

두 번째 SQL은 __두 번째 SQL은 학생 테이블의 행 수__을 세고 있었다.

따라서 SQL을 검토할 때는 문법보다 먼저__어떤 데이터를 세는 SQL인지__를 확인해야 한다.
```
...
![STEP 4 중복 데이터 합계 결과](./images/step04_duplicate_result.png)
![STEP 4 중복 데이터 합계 결과](./images/step04_duplicate_result2.png)
---
...
| --- | --- | --- |
| 사용자 행동 기록을 별도로 관리 | 수용 | 추천 결과를 개선하려면 수락·거절·완료 기록이 필요하다고 판단했다. |
| 퀘스트와 사용자에게 배정된 퀘스트를 구분 | 수정 | 필요성은 이해했지만 아직 데이터 구조를 확정할 단계는 아니라고 판단했다. |
| 초기 추천은 점수 기반 방식으로 구현 | 수용 | 초보 단계에서도 구현 가능하고 추천 이유를 확인하기 쉽기 때문이다. |
| 사용자의 행동을 이용해 목표를 자동 결정 | 수정 | 시스템이 확정하기보다 목표를 추천하고 사용자가 선택하는 방식이 적절하다고 판단했다. |
| Next.js, Python 백엔드, PostgreSQL을 기본 구조로 사용 | 수용 | 현재 기획의 MVP를 구현하기에 필요한 기본 구성이라고 판단했다. |
...
```text
처음에는 추천 시스템에 필요한 구조를 바로 제안했지만,
다시 검토하면서 실제 서비스 정책이 정해지지 않은 부분은 먼저 확인해야 한다고 판단했다.
```
...
사용자의 행동 기록을 추천 알고리즘에 활용할 수 있다고 전제한 점.
```

추가로 업무 담당자에게 확인해야 한다고 판단한 내용:

```text
퀘스트가 사용자마다 개별적으로 생성되는지,
같은 퀘스트를 여러 사용자에게 추천할 수 있는지,
사용자 행동 기록을 어느 범위까지 추천에 활용할 것인지.
```
