...
- 대상: 어떤 상품의 어떤 서비스의 매출이 줄어들었는지 파악이 되지 않기때문
- 기간: 매출이 줄어들기 시작하고 언제까지의 기간에 매출이 줄어들었는지 명확하지 않음
- 기준: 순이익, 일반 매출 같은 기준이 없음
- 비교 방법: 기간, 제품 분류, 구매고객 분류 등 어떻게 비교해야 하는지가 없음
...
- 연령, 성별 기준으로 분류하고, 2025년1월부터 분기(3개월)단위로 기간별로 2026년 3분기(6-8월) 비교 분석한다.
- 우선 비교 항목은 분기 > 성별 > 연령 순서로 한다.
- 스포츠 카테고리 안에서도 분류별 분기> 성별 > 연령 순서로 비교한다.
